# ADR-ORDSA-0005 — ArchiMate as canonical visual notation; pyArchimate as reference renderer
...
OrdSA needs a canonical visual notation for layer diagrams, layer-coverage matrices, conformance examples, and adoption-story figures. Per [ADR-ORDSA-0001](ADR-ORDSA-0001-schema-first-canonical-form.md), the schema is the source of truth and visual renderings are derived from it. Per [ADR-ORDSA-0004](ADR-ORDSA-0004-reference-tooling-python-pydantic.md), reference tooling is Python.

The choice of visual notation locks two things: (a) which enterprise-architecture audiences see OrdSA in "their" language, and (b) what tooling sits in the rendering pipeline. Naively, the answer would be "use Archi-the-desktop-tool with ArchiMate-the-language." Research surfaced two facts that change that picture:
...
Combined: the schema-first principle (ADR-ORDSA-0001) is best served by *generating diagrams from the schema at build time*, not by hand-editing them in a GUI. pyArchimate enables that. Archi-the-desktop-tool becomes an optional consumer of our artifacts, not a producer in the pipeline.

## Decision

**ArchiMate 3.2 is the canonical visual notation for OrdSA renderings.** Diagrams in the repo are produced by build-time tooling from the canonical YAML schema, not authored by hand in a GUI.

**The reference rendering pipeline:**

1. **YAML construct/deployment schema** (per ADR-ORDSA-0001/0002/0003) is the source.
2. Reference Python tooling (per ADR-ORDSA-0004) loads the schema and constructs a **pyArchimate** object model.
...
**Archi-the-desktop-tool is OPTIONAL** for OrdSA contributors and consumers. Users who prefer GUI editing/exploration of an ArchiMate model can open the Open Exchange XML we emit. They are not in the build pipeline.
...
- **No GUI tool is load-bearing.** If Archi's stewardship lapses, OrdSA's pipeline keeps working; if pyArchimate's GPL posture changes, the Archimate-PlantUML+Kroki fallback exists.
...
- **pyArchimate auto-layout quality on dense OrdSA graphs is unverified.** Pilot during the v0.2 schema PR — if layout degrades badly for the seven-layer stack, the fallback may need to come forward.
...
3. **pyArchimate imported as a library (not subprocess).** Cleanest from a code-architecture perspective. Rejected — GPL-3.0 would impose GPL on the importing code, conflicting with [ADR-ORDSA-0004](ADR-ORDSA-0004-reference-tooling-python-pydantic.md)'s Apache-2.0 choice. The subprocess boundary preserves both.
...
7. **No designated rendering pipeline.** Adopters render diagrams themselves in whatever tool they prefer. Rejected — without canonical diagrams in the repo, the prose is the only OrdSA artifact most readers will see. Visualizable OrdSA matters for adoption.

## References

- [ADR-ORDSA-0001](ADR-ORDSA-0001-schema-first-canonical-form.md) — schema-first canonical form (the principle this ADR operationalizes for visuals)
- [ADR-ORDSA-0002](ADR-ORDSA-0002-schema-language-yaml-json-schema.md) — YAML + JSON Schema (the input to the rendering pipeline)
- [ADR-ORDSA-0004](ADR-ORDSA-0004-reference-tooling-python-pydantic.md) — Python + pydantic reference tooling (the host of the subprocess invocation)
